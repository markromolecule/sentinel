...
describe('ActionQueueColumns in sentinel-core', () => {
...
describe('ActionQueuePanel Multi-Select & Batch Toolbar in sentinel-core', () => {
...
describe('ActionQueueView Tabs in sentinel-core', () => {
    it('renders tabs with counts and switches queues cleanly', () => {
        const onGrantOverride = vi.fn();
        render(
            <ActionQueueView
                actionQueues={{
                    review: [],
        ...
        expect(screen.getByText('Needs Review (0)')).toBeDefined();
        expect(screen.getByText('Needs Makeup (2)')).toBeDefined();
        expect(screen.getByText('Needs Retake (0)')).toBeDefined();

        // Active queue defaults to makeup
        expect(screen.getByText('Smith, Alice')).toBeDefined();

        // Switch to review queue
        fireEvent.click(screen.getByText('Needs Review (0)'));
        expect(screen.getByText(/No students in this queue right now/i)).toBeDefined();
    });
});
