...
import { EVIDENCE_DECISION_TIMEOUT_MS, MAX_PENDING_EVIDENCE_DECISIONS } from '../_constants';
...
export function useIncidentTelemetryDispatcher(): UseIncidentTelemetryDispatcherResult {
        ...
        async ({
            ...
            const timeoutId = window.setTimeout(() => {
                decisionController.abort(
                    new DOMException('Evidence decision timed out.', 'AbortError'),
                );
    ...
    const clearInFlightEvents = useCallback(() => {
        for (const controller of inFlightEvidenceDecisionsRef.current.values()) {
            controller.abort(
                new DOMException('Evidence decision aborted during cleanup.', 'AbortError'),
            );
        }
