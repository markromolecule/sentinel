...
export class AnalyticsOverallDocumentProcessor implements PdfDocumentProcessor {
    ...
    async render(
        ...
        // Resolve the creator's display name for the "Generated By" field
        let generatedBy = 'Sentinel Analytics System';
        if (exportRecord.created_by && exportRecord.created_by !== 'system-worker') {
            try {
                const creator = await dbClient
                    .selectFrom('auth.users as u')
                    .leftJoin('user_profiles as up', 'up.user_id', 'u.id')
                    .select(['u.email', 'up.first_name', 'up.last_name'])
                    .where('u.id', '=', exportRecord.created_by)
                    .executeTakeFirst();
                if (creator) {
                    const name = [creator.first_name, creator.last_name].filter(Boolean).join(' ');
                    generatedBy = name || creator.email || generatedBy;
                }
            } catch {
