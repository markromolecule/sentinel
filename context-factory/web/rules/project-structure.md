...
│   └── use-<hook-name>/    # Hook directory structure
│       ├── index.ts        # Main hook implementation
│       ├── _types.ts       # Hook-specific types (REQUIRED)
│       └── _constants.ts   # Hook-specific constants (if needed)
...
**Important**: Always separate types and constants from implementation. See `code-organization.mdc` for detailed guidelines.
...
│   └── _types.ts           # Action-specific types (REQUIRED if actions exist)
├── _components/            # Route's components
│   └── _types.ts           # Component-specific types (REQUIRED)
├── _constants/             # Route's constants (REQUIRED)
│   └── index.ts
...
│   └── _types.ts           # Component-specific types (REQUIRED)
├── _constants/             # Feature's constants (REQUIRED)
│   └── index.ts
...
├── _types/                 # Feature's types (REQUIRED)
│   └── index.ts
