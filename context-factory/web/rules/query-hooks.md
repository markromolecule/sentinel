...
import { getSampleData, type GetSampleDataArgs } from '@/data/samples/get-sample';
...
vi.mock('@/data/samples/get-sample', () => ({
  getSampleData: vi.fn(),
}));
...
describe('useSampleQuery', () => {
  ...
  it('should fetch sample data', async () => {
    vi.mocked(getSampleData).mockResolvedValueOnce(mockSample);
    ...
    expect(getSampleData).toHaveBeenCalledWith({ id: mockSample.id });
    expect(getSampleData).toHaveBeenCalledTimes(1);
    expect(result.current.data).toEqual(mockSample);
  });

  it('should handle error state', async () => {
    const error = new Error('Failed to fetch sample');
    vi.mocked(getSampleData).mockRejectedValueOnce(error);
  ...
  it('should not fetch when id is not provided', () => {
    const { result } = renderTestHook({ id: '' });
