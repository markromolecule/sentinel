...
4. Essay questions are evaluated using the standard `essay-rubric` module with deterministic weighted scoring, eliminating Gemini token consumption and rate-limit dependencies while giving instructors full slider calibration (Levels 0–4).
...
### Actors and Goals
- **Instructor (Exam Proctor / Grader):** Wants accurate live submission counts and real-time student progress during live exams; wants a streamlined, standardized multi-criterion rubric slider interface (Levels 0–4) to grade essays deterministically with 0 token consumption; wants clear, truthful progress metrics on the grading dashboard.
...
|---|---|---|---|---|---|
| SC-01 | Student with 5 flags submits exam | Student has flagged telemetry incidents and clicks submit | Monitoring header increments `Submitted` count; student card displays both `SUBMITTED` lifecycle badge and `Flagged` badge | Filter by "Submitted" or "Flagged" properly includes student | Completed |
| SC-02 | Instructor views grading table with 1/46 submissions | 1 student submitted and graded out of 46 enrolled | Progress bar displays proportional fill (~2%), label shows `1/1 graded • 1/46 submitted` | If total is 0, progress bar gracefully shows 0% | Completed |
| SC-03 | Student answers questions during live exam | Exam in progress, instructor on monitoring page | Progress bar on student card updates in real time (<50ms) via Supabase Realtime Broadcast without DB queries | Falls back to polling overview if broadcast dropped | Completed |
| SC-04 | Instructor opens essay grading workspace | Student has submitted essay responses | Instructor evaluates essays using the standardized `essay-rubric` criteria sliders (0–4) with instant mathematical weighted score calculations | Sliders default to standard baseline; scores calculated deterministically | Completed |
...
| DEC-04 | Essay Grading Engine: AI vs Deterministic Rubric | Rely exclusively on the standard `essay-rubric` module with weighted calculation; completely clean up AI grading code | 0 Gemini token consumption, zero quota/cost overhead, eliminates hallucination risk, fully deterministic | LLM token consumption and external API dependency | `phase-04` |
...
| AC-05 | SC-04 / DEC-04 | Essay responses are graded using the multi-criterion rubric sliders (0-4) with real-time weighted score calculation and 0 AI token usage | `GradingRubricPane.tsx` + `calculateEssayWeightedScore` | Component tests + Hook tests + Shared package tests | Passed |
...
- [x] `phase-04-essay-rubric-ai-prescoring.md` — Phase 4: Standard Essay Rubric Module Integration & AI Cleanup
