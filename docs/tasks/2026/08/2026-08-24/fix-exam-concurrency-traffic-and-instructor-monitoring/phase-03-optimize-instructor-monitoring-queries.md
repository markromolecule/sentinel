...
- [ ] **Task 3.1: Relax Instructor Monitoring Overview Refetch Interval**
  - Set `EXAM_MONITORING_OVERVIEW_REFETCH_INTERVAL_MS = 6000`.
  - Set `refetchIntervalInBackground: false` and `staleTime: 4000`.
- [ ] **Task 3.2: Relax Instructor Incidents Refetch Interval**
  - Set `EXAM_INCIDENTS_REFETCH_INTERVAL_MS = 6000`.
  - Set `refetchIntervalInBackground: false` and `staleTime: 4000`.
- [ ] **Task 3.3: Relax Student Detail Monitoring Query**
  - In `useExamMonitoringStudentQuery`, set `refetchInterval: 8000` and `refetchIntervalInBackground: false`.
- [ ] **Task 3.4: Execute Instructor Monitoring Hook Test Suites**
...
```bash
pnpm --filter @sentinel/hooks test use-exam-monitoring-overview-query.test.ts
pnpm --filter @sentinel/hooks test use-exam-incidents-query.test.ts
pnpm --filter @sentinel/hooks test use-exam-monitoring-student-query.test.ts
```
