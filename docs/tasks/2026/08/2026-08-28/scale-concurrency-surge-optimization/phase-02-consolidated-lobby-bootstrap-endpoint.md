...
2. **[`app/sentinel-api/src/modules/examination/lobby/services/bootstrap-lobby.ts`](file:///Applications/XAMPP/xamppfiles/htdocs/sentinel/app/sentinel-api/src/modules/examination/lobby/services/bootstrap-lobby.ts)** [NEW]
   ...
   - Return `{ exam, configuration, admission, waitingCount, runtimeAccess }`.
...
5. **[`packages/services/src/api/exams/lobby.ts`](file:///Applications/XAMPP/xamppfiles/htdocs/sentinel/packages/services/src/api/exams/lobby.ts)**
   - Add `bootstrapExamLobby(apiClient, examId)` client service function and `ExamLobbyBootstrapResult` type.

---

## Implementation Tasks

- [x] **Task 2.1 — Strip Synchronous Admin Audit Notifications from Check-in**
...
- [x] **Task 2.2 — Implement `bootstrapLobby` Service (`bootstrap-lobby.ts`)**
...
- [x] **Task 2.3 — Create Route & Controller (`bootstrap-lobby.controller.ts`)**
  - Implement OpenAPI route definition and controller.
  - Wire into `lobby.routes.ts`.

- [x] **Task 2.4 — Export Client Service (`packages/services/src/api/exams/lobby.ts`)**
...
# Result: 7 test files, 29 tests passed

# 2. Test contracts
pnpm --filter sentinel-api test 'src/tests/exams/exam-contracts.test.ts'
# Result: 1 test file, 23 tests passed

# 3. Build services package
