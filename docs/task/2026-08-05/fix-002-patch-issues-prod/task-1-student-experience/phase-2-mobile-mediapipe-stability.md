...
- [x] Add temporary or structured diagnostics for face count, video dimensions, frame timestamp, detector/session token, and stage; exclude image bytes, landmarks, and student-identifying data.
...
- [x] Add the smallest confirmed mobile-input normalization in the web hook/runtime; do not force `numFaces: 1` or suppress real events.
...
- [x] Run focused web/shared tests and record the results here.
- [ ] Run a device smoke test on iOS Safari and Android Chrome in portrait mode.
...
- Added redacted per-frame diagnostics for the checkup and attempt MediaPipe loops, including stage, session token, detector token, video dimensions, frame timestamp, and face count.
- Normalized landmark precision before analysis to smooth noisy mobile input without suppressing real multi-face detections.
...
- [x] Focused tests and redacted device diagnostics are recorded here.
