...
- [x] Confirm Phase 1 schema, shared contract, RBAC, and migration validation is complete.

## Tasks

- [x] Refactor `getExamReport()` in
...
- [x] Execute the source reads through one database transaction/consistent snapshot in
...
- [x] Add a clearly sample-labelled fixture at
...
- [x] In `exam-results-report-renderer.ts`, add an unconditional `doc.addPage()` after the student
...
- [x] In `exam-results-report-renderer.ts`, calculate row heights before drawing, repeat table
...
- [x] Add a large-cohort renderer case (at least 250 students with long names/sections) to
...
- [x] Run targeted ESLint on every new/changed Phase 2 TypeScript file and verify all exported
      helpers have JSDoc.
