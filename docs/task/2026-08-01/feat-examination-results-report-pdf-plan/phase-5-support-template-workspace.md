...
- [ ] Confirm Phase 4 shared client packages pass tests and builds.

## Tasks

- [ ] Add `examination-reports` to `PdfTemplateSection` and `NAV_ITEMS` in
...
- [ ] Create
      ...
      `EXAM_RESULTS_REPORT`.
- [ ] In the new page, provide global fallback plus accessible institution override selection
      ...
      export permission.
- [ ] Extend `previewPdfTemplateBodySchema` and
...
- [ ] In `preview-pdf-template.controller.ts`, apply the same global/institution template scope
...
- [ ] Verify the catalog-driven Permission Registry and Role Matrix render
...
- [ ] Run `pnpm --dir app/sentinel-support exec vitest run 'src/app/(protected)/(support)/pdf-templates'`.
- [ ] Run the focused API preview controller test for `EXAM_RESULTS_REPORT`.
- [ ] Run `pnpm --dir app/sentinel-support lint` and `pnpm --dir app/sentinel-support build`.
- [ ] Manually preview global and institution override configurations and inspect header, footer,
