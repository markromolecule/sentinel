...
- [x] Confirm Support template preview/publish and Phase 5 tests pass.

## Tasks

- [x] Add an `ExamReportPdfExport` client component under
...
- [x] Render the Web export component in
...
- [x] Add the equivalent component at
...
- [x] Render it in `app/sentinel-core/src/app/(protected)/exams/[id]/report/page.tsx` beside Refresh
...
- [x] Handle API `403` after a stale client permission by closing/disabling export actions,
...
- [x] Ensure signed URLs are opened with `noopener,noreferrer`, are requested only on explicit
...
- [x] Run `pnpm --dir app/sentinel-web exec vitest run 'src/app/(protected)/(instructor)/exams/reports/[examId]/_components/exam-report-pdf-export.test.tsx' 'src/app/(protected)/(instructor)/exams/reports/[examId]/page.test.tsx'`.
- [x] Run `pnpm --dir app/sentinel-core exec vitest run 'src/app/(protected)/exams/[id]/report/_components/exam-report-pdf-export.test.tsx' 'src/app/(protected)/exams/[id]/report/page.test.tsx'`.
...
Targeted Phase 6 Vitest coverage passed on August 1, 2026 for the new Web/Core export components and
their report-page integrations. Broader `tsc --noEmit` checks in both apps still report unrelated
pre-existing test-suite type errors outside this Phase 6 surface, and scoped ESLint runs still include
existing `no-explicit-any` violations in the surrounding report-page test files, so the broader
lint/type/build validation item remains open.
