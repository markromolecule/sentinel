...
- [x] Extend `DocumentKindSchema` in
...
- [x] Add examination-report export lifecycle schemas and inferred types in
...
- [x] Extend `packages/shared/src/schema/pdf-documents/pdf-document-schema.test.ts` for the new
...
- [x] Extend `documentKindEnum`, template preview/draft DTOs, and examination-report create/list/
...
- [x] Add `EXAM_RESULTS_REPORT` built-in header/footer defaults to
      ...
      fallback precedence.
- [x] Add `EXAM_RESULTS_REPORT` template-resolution coverage in a new colocated test
...
- [x] Register `EXAMINATIONS_EXPORT_RESULTS_REPORT` with key
...
- [x] Add `exam_report_exports` to `packages/db/prisma/schema.prisma` with `export_id`, `exam_id`,
...
- [x] Add migration comments documenting allowed status values and the private-artifact expectation;
...
- [x] Regenerate Prisma/Kysely output so `exam_report_exports` is present in
...
- [x] Update every exhaustive document-kind union in
...
- [x] Run `pnpm --dir packages/db generate` and confirm generated output contains
      `exam_report_exports`.
- [x] Run `pnpm --dir packages/shared test -- src/schema/pdf-documents/pdf-document-schema.test.ts`.
- [x] Run `pnpm --dir app/sentinel-api exec vitest run src/modules/security/permission/data/sync-system-permissions.test.ts src/modules/general/pdf-documents/data/pdf-document-schema.integration.test.ts src/modules/general/pdf-documents/services/resolve-pdf-template.service.test.ts`.
- [x] Run `pnpm --dir packages/shared lint` and `pnpm --dir app/sentinel-api lint` for the changed
