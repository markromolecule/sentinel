...
- [x] Confirm Phase 1 source mapping and selected-exam preview tests/visual checks pass.

## Tasks

- [x] Change `createAnswerKeyExportBodySchema` in
...
- [x] Replace generic template/report permission alternatives with
...
- [x] In the list controller, derive the allowed institution/exam predicate from actor context even
...
- [x] In retry, require `FAILED`; in download, require `READY`, storage coordinates, and non-expiry;
...
- [x] Remove raw `storageBucket`/`storagePath` from `answerKeyExportRecordSchema` in
...
- [x] Add/extend a colocated test for each answer-key controller covering dedicated permission,
...
- [x] Add `examinations:export_answer_key` to superadmin, admin, and instructor defaults (Support
...
- [x] Update answer-key service tests in `packages/services/src/api/pdf-documents.test.ts` for the
      exam-only create payload and public record without storage coordinates.
- [x] Update `useCreateAnswerKeyExportMutation()` and answer-key query invalidation in
...
- [x] Expand Support component/page tests for a template manager without answer-key permission, an
...
- [x] Run focused answer-key controller, scope integration, and API integration tests in
      `app/sentinel-api`.
- [x] Run `pnpm --dir app/sentinel-api exec vitest run src/modules/security/permission/data/sync-system-permissions.test.ts`.
- [x] Run focused `packages/services`, `packages/hooks`, and Support answer-key panel/page tests.
- [x] Run targeted lint/build validation for API, shared, services, hooks, and Support.
