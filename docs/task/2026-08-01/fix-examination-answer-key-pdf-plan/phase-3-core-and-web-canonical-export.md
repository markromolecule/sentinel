...
- [ ] Create or extend the shared controlled component at
...
- [ ] Add/extend `packages/ui/src/components/pdf-export-lifecycle-panel.test.tsx` for terminal
...
- [ ] Update both `app/sentinel-web/src/features/exams/_hooks/use-exam-card/index.ts` and
...
- [ ] Extend both `use-exam-card/index.test.tsx` files for granted/revoked permission across draft,
      published/active, archived, and fallback statuses, plus exact label/href.
- [ ] Add or replace route tests beside both `export/page.tsx` files for permission denied, explicit
...
- [ ] Open signed downloads with `noopener,noreferrer`, request them only after a user click, and do
      not persist URLs in local/session storage or toasts in either route.
- [ ] After both new route suites pass, delete
...
- [ ] Remove obsolete export-folder barrel imports if present and run
...
- [ ] Preserve correct-answer confidentiality by asserting in both route tests that page data comes
...
- [ ] Run focused Web exam-card and `/exams/[id]/export` route tests.
- [ ] Run focused Core exam-card and `/exams/[id]/export` route tests.
- [ ] Run the `packages/ui` lifecycle component test and targeted `packages/hooks` answer-key tests.
- [ ] Run targeted lint/build validation in `sentinel-web` and `sentinel-core`.
